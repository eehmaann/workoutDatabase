<html>
   ...
   <body>
    	...
    	<cfif emptyMeasurement.RecordCount gt 0>
    		...
    		<div class="aform">
    			<p> Add Measurement</p>
    				<cfform action="addMeasurement.cfm" method="post" scriptSrc=
                    "http://cscie60.dce.harvard.edu:8500/CFIDE/scripts/"
                    perservedata ="Yes">
 						What day did you measure    
                        <select name="workoutNo">
                			<cfoutput query="emptyMeasurement">
                  				<option value="#emptyMeasurement.workoutNo#">
                                #emptyMeasurement.workdate#</option>
                			</cfoutput>
              			</select><br>
 					What program was used   
                    	<select name="programNo">
                			<cfoutput query="showPrograms">
                  				<option value="#showPrograms.programNo#">
                                #showPrograms.programName#</option>
                			</cfoutput>
              			</select><br>
                        Enter your weight 
                        <cfinput type="text" size="3" name="weight" 
                        validate="integer" required="yes" 
                        message="Your weight must be typed as an integer">
                        
						Enter the circumference of your arm at the bicep 
                        <cfinput type="text" size="3" name="armSize" validate=
                        "integer" required="yes" message="Your arm size must be
                         typed as a integer">
                         
						Enter the circumference of your leg around the thigh
                         <cfinput type="text" size="3" name="legSize" 
                         validate="integer" required="yes" 
                         message="Your leg size must be type as an integer"><br>
                         
						Enter the circumference around the chest 
                        <cfinput type="text" size="3" name="chestSize" 
                        validate= "integer" required="yes" 
                        message="Your chest size is a required integer">

						Enter the circumference around your waist 
                        <cfinput type="text" size="3" name="waistSize" 
                        validate= "integer"required="yes" 
                        message="Your waist size is a required integer"><br>
                        <cfinput type="submit" name="addMeasurement" 
        ...
        <cfif showMeasurements.RecordCount gt 1>
        	<cflayoutarea title="More Options">        
       			...
       			<p>  
                <a href="ShowPrograms.cfm"> View all programs/ Create a new one</a>
                </p>
      			
                <p> Look at a specific program
                 <cfform action="MeasurementbyProgram.cfm" method="post" 
